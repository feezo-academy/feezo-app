...
export default function HomeTab() {
  ...
  const chartData = useMemo(() => dateRange.map(dateStr => {
    ...
    const present = dayRows.filter(a => String(a.status).toUpperCase() === 'P').length;
    const absent = dayRows.filter(a => String(a.status).toUpperCase() === 'A').length;
  ...
  // Dashboard tiles are scoped to the selected sport/batch filters.
  // Total Students is the complete filtered roster; Active strength remains a chart concept.
  ...
  // Never fall back to another month. A selected month with no matching records means no fee data.
  const scopedFees = scopedFeesAll.filter(feeMatchesMonth);

  const collectedFees = scopedFees.filter(f => String(f.status).toLowerCase() === 'paid');
  const pendingFees = scopedFees.filter(f => String(f.status).toLowerCase() !== 'paid');
  ...
  const pending = pendingStudentIds.size;
