...
export default function SportsBatchesPage() {
  ...
  const [newBatchNames, setNewBatchNames] = useState({}); // sport.id -> draft batch name
  ...
  const addBatch = async (sport) => {
    const draft = (newBatchNames[sport.id] || '').trim();
    if (!draft) return;
    const { error: err } = await supabase.from('batches').insert({ name: buildBatchKey(sport.name, draft), academy_id: academyId });
    if (err) { setError(err.message); return; }
    setError('');
